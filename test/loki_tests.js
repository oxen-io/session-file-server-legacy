...
const config       = require('../loki/lib.config.js');
const adnServerAPI = require('../loki/server/fetchWrapper');
  ...
  return new Promise((resolve, rej) => {
    ...
    lokinet.portIsFree('localhost', webport, function(free) {
      ...
      if (free) {
        ...
        const startPlatform = require('../file_server.js');
      } else {
...
const runIntegrationTests = async (ourKey, ourPubKeyHex) => {
  ...
  describe('token dialect', () => {
    require('../loki/test/tests/tokens/get_challenge.js')(testInfo);
    require('../loki/test/tests/tokens/submit_challenge.js')(testInfo);
    ...
    require('../loki/test/tests/tokens/user_info.js')(testInfo);
  });
  describe('file server dialects', () => {
